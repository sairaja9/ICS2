  ...
  if user_input not in user_input_list:
    ...
    if room_contents == 'monster' and user_room <= 3:
      ...
      if temp <= 4:
        ...
        board[user_room] = ' '
        floor_numbers = random.choice(floor_list)
        user_room = random.choice(room_list)
        pocket = []
        lives -= 1
    else:
    ...
    if room_contents == 'boss monster' and ("A sword" in pocket and "Some stones" in pocket):
      ...
      win = True
    elif room_contents == 'boss monster' and ("A sword" not in pocket or "Some stones" not in pocket):
    ...
    if room_contents == 'stones' and len(pocket) < 3:
      ...
      print("Congrats!! You won the final prize!")
      init = False
    elif len(pocket) >= 3:
